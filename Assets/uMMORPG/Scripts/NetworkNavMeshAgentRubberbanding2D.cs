...
public class NetworkNavMeshAgentRubberbanding2D : NetworkBehaviour
{
    ...
    Vector2 lastSentDestination;
    Vector2 lastSentPosition;
    Vector2 lastReceivedDestination;
    float lastSentTime;
    bool hadPath;
    ...
    [Command]
    void CmdMovedClick(Vector2 destination, float stoppingDistance)
    {
    ...
    [Command]
    void CmdMovedWASD(Vector2 position)
    {
        // rubberband (check if valid move)
    ...
    void Update()
    {
        ...
        if (isServer)
        {
            ...
            if (!hasPath && agent.velocity == Vector2.zero &&
                Vector2.Distance(transform.position, lastServerPosition) > agent.speed)
            {
            ...
            if (HasPath() && Vector2.Distance(agent.destination, lastReceivedDestination) > epsilon)
            {
            ...
            // (epsilon comparison needed for float precision over the network)
            if (hadPath && !hasPath && Vector2.Distance(transform.position, lastReceivedDestination) > epsilon)
            {
                // set dirty so onserialize notifies others
                SetDirtyBit(1);
        ...
        if (isLocalPlayer)
        {
            ...
            else if (!hasPath && agent.velocity != Vector2.zero && (Vector2)transform.position != lastSentPosition)
            {
                // don't send all the time to not DDOS the server
                // note: if we check time BEFORE detecting move then moves are
                //       detected very late and cause additional latency and
    ...
    [TargetRpc]
    void TargetSetDestination(NetworkConnection conn, Vector2 destination, float stoppingDistance)
    {
        agent.stoppingDistance = stoppingDistance;
        agent.destination = destination;
    }
